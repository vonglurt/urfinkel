...
# stays hand-written.
splice() {  # splice <file> <block> <out>
    if ! grep -qF "$BEGIN" "$1" || ! grep -qF "$END" "$1"; then
        echo "checksums: $1 has no $BEGIN / $END markers" >&2
        exit 1
    fi
    python3 - "$1" "$2" "$BEGIN" "$END" > "$3" <<'PY'
import sys
...
if [ "$CHECK" = "1" ]; then
    if [ "$stale" = "1" ]; then
        echo "checksums: the manifest, the sidecars, $README or $PLAYPAGE do" >&2
...
if [ "$stale" = "1" ]; then
    echo "checksums: manifest, sidecars, $README and $PLAYPAGE updated ($stamp)"
else
